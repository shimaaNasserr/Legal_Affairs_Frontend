...
export default function FatwasPage() {
  ...
  const buildFormData = () => {
    ...
    if (!editingId && userDepartmentId)
      fd.append("department", userDepartmentId);
    return fd;
  };
        ...
        <div className="col-lg-6 mb-3">
          <div className="card">
            <div className="card-header">
              {editingId ? "تعديل فتوى" : "إضافة فتوى"}
            </div>
              ...
              <form onSubmit={editingId ? handleUpdate : handleCreate}>
                ...
                {!editingId && !userDepartmentId && (
                  <div className="alert alert-warning py-2">
                    لا توجد إدارة للمستخدم. لا يمكن إنشاء فتوى دون تحديد
                    الإدارة.
                  </div>
                )}
                <div className="d-flex gap-2">
                  <button
                    disabled={loading || (!editingId && !userDepartmentId)}
                    type="submit"
                  ...
                  <tbody>
                    ...
                    {pagedFatwas.map((f) => (
                      <tr key={f.id}>
                        ...
                        <td title={f.request_content}>
                          {(f.request_content || "").slice(0, 40)}
                          {(f.request_content || "").length > 40 ? "…" : ""}
                        </td>
                        <td>
